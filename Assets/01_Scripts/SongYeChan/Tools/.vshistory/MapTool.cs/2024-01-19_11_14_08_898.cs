        ...
        for (int i = 0; i < mapY; i++)
        {
            for (int j = 0; j < mapX; j++)
            {
                ...
                else if (mapInfo[i][j] == 2)
                {
                    ...
                    obTreeObject.transform.position = new Vector3(x * objScale * 10, k == 0 ? planeObject.transform.position.y + objScale * 5 : (prevCreatedYPos + objScale * 10), z * objScale * 10);
                    obStoobTreeObjectneObject.transform.localScale = new Vector3(objScale * 10, objScale * 10, objScale * 10);
                }
