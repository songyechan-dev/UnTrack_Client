...
public class MapTool : EditorWindow
{
    ...
    private void CSVLoad()
    {
        ...
        for (int i = 0; i < lines.Length; i += 2) // 두 개씩 건너뛰면서 처리
        {
            string[] values1 = lines[i].Split(',');
            string[] values2 = lines[i + 1].Split(',');
            if (values1.Length >= 2 && values2.Length >= 2)
            {
                string startRotationKey1 = "startTrackYRotation";
                float endRotation1 = float.Parse(values1[1]);
                string startRotationKey2 = "endTrackYRotation";
                float endRotation2 = float.Parse(values2[1]);
                rotationInfoDict.Add(startRotationKey1, endRotation1);
                rotationInfoDict.Add(startRotationKey2, endRotation2);
            }
            else
            {
                Debug.LogError("Invalid CSV format: " + lines[i]);
            }
        }
    }
