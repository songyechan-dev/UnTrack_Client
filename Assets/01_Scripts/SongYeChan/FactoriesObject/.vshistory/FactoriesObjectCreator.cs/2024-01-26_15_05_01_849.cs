...
public class FactoriesObjectCreator : MonoBehaviour
{
    ...
    public string factoriesObjectPrefabName = "factoriesObject";
    ...
    public void Create()
    {
        if (GameManager.Instance().gameState.Equals(GameManager.GameState.GameStart))
        {
            ...
            GameObject factoriesObject = PhotonNetwork.Instantiate(factoriesObjectPrefabName, new Vector3(MapInfo.defaultStartTrackX * MapInfo.objScale * 10, (factoriesObjectPrefab.transform.localScale.y / 2) + (MapInfo.trackYscale / 2) * 2, MapInfo.defaultStartTrackZ * MapInfo.objScale * 10),Quaternion.Euler(new Vector3(0, MapInfo.startTrackYRotation, 0)));
