...
public class FactoriesObjectManager : MonoBehaviour
{
    ...
    void RotationToDirection()
    {
        ...
        if (isChangedRotation)
        {
            ...
            if (transform.forward == lockedHit2.transform.position)
            {
                Debug.Log("일치함");
            }
        ...
        else if ((moveDirection.Equals(MoveDirection.LEFT) || moveDirection.Equals(MoveDirection.RIGHT)) && !isStop)
        {
            ...
            if (Physics.Raycast(transform.position, Vector3.down, out lockedHit2))
            {
                if (lockedHit2.transform.CompareTag(tagToBeDetected))
                {
                    targetTransform = lockedHit2.transform;
