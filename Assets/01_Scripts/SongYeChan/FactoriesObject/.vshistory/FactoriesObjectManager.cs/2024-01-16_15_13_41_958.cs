...
public class FactoriesObjectManager : MonoBehaviour
{
    ...
    {
        StartCoroutine(RepeatTurnFinishPosCorrection());
    ...
    IEnumerator RepeatTurnFinishPosCorrection()
    {
        while (true)
        {
            TurnFinishPosCorrection();
            Vector3 targetPosition = targetTransform.position + new Vector3(0, targetTransform.localScale.y / 2 + transform.localScale.y / 2, 0);
            yield return StartCoroutine(MoveToTargetPosition(targetPosition, 1f, 1));
            break;
        }
    }

    void TurnFinishPosCorrection()
    {
    ...
    IEnumerator MoveToTargetPosition(Vector3 targetPosition, float duration, int steps)
    {
