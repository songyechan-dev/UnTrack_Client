...
public class FactoriesObjectManager : MonoBehaviourPun
{
    ...
    private void OnTriggerEnter(Collider other)
    {
        if (other.transform.CompareTag(transform.tag))
        {
            ...
            //사운드 재생시켜야됨
            GameManager.Instance().SleepCoroutine(1f, () => DestroyMyObject(other.transform.GetComponent<PhotonView>().ViewID));
        }
    }
    public void DestroyMyObject(int _targetViewID)
    {
        if (PhotonNetwork.IsMasterClient)
        {
            ...
            PhotonNetwork.Destroy(PhotonView.Find(_targetViewID).gameObject);
        }
    }
}
