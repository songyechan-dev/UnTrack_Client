...
public class FactoriesObjectManager : MonoBehaviour
{
    ...
    void TurnFinsh(float _finishedAngle)
    {
        ...
        float lerpFactor = Mathf.Clamp01(Time.deltaTime * moveSpeed);
        transform.position = Vector3.Lerp(transform.position, targetTransform.position + new Vector3(0, targetTransform.localScale.y / 2 + transform.localScale.y / 2, 0), lerpFactor);
