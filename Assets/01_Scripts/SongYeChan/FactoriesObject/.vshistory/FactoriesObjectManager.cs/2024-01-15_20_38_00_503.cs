...
public class FactoriesObjectManager : MonoBehaviour
{
    ...
    void CheckDirection()
    {
        ...
        if (moveDirection == MoveDirection.FORWARD)
        {
            if (isChangedRotation)
            {
                ...
                if (transform.rotation.eulerAngles.y <= lessAngle || transform.rotation.eulerAngles.y < 270f)
                {
                    isStop = true;
                    moveDirection = MoveDirection.FORWARD;
                    myState = MyState.STOP;
    ...
    IEnumerator SetIsStop()
    {
        ...
        myState = MyState.MOVE;
        isStop = false;
        transform.position = hit.transform.position + new Vector3(0,(transform.localScale.y /2) + hit.transform.localScale.y / 2,0);
