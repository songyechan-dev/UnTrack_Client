...
public class FactoriesObjectManager : MonoBehaviour
{
    ...
    /// </summary>
    void TurnFinish(float _finishedAngle)
    {
    ...
    IEnumerator RepeatTurnFinishPosCorrection()
    {
        while (true)
        {
            ...
            yield return StartCoroutine(MoveToTargetPosition(targetTransform.position + new Vector3(0, targetTransform.localScale.y / 2 + transform.localScale.y / 2, 0), 5f));
