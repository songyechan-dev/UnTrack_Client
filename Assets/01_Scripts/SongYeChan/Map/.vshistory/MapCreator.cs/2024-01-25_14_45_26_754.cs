            ...
            for (int j = 0; j < mapX; j++)
            {
                ...
                else if (mapInfo[i][j] == (int)MapInfo.Type.FACTORY)
                {
                    ...
                    else
                    {
                        ...
                        {
                            pv.RPC("CreateFactory_Master", RpcTarget.MasterClient, x, y, z, name, FACTORYTYPE.ProductionMachine);
                        }
                        else if (StateManager.Instance().waterTanks.Count > 0)
                        {
                            pv.RPC("CreateFactory_Master", RpcTarget.MasterClient, x, y, z, name, FACTORYTYPE.WaterTank);
                        }
                        else if (StateManager.Instance().dynamiteMachines.Count > 0)
                        {
                            pv.RPC("CreateFactory_Master", RpcTarget.MasterClient, x, y, z, name, FACTORYTYPE.DynamiteMachine);
                        }
                    }
                }
