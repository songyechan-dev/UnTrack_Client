...
public class MapCreator : MonoBehaviour
{
        ...
        for (int i = 0; i < mapY; i++)
        {
            for (int j = 0; j < mapX; j++)
            {
                ...
                if (mapInfo[i][j] == (int)MapInfo.Type.OBSTACLE_STONE)
                {
                    ...
                    {
                        pv.RPC("CreateObject_Others", RpcTarget.Others, x, y, z, obStonePrefab.name, k, prevCreatedYPos);
    ...
    [PunRPC]
    void CreateObject_Master(float x, float y, float z, string name, int k,float prevCreatedYPos)
    {
        Debug.Log("마스터 호출");
        ...
        pv.RPC("CreateObject_Others", RpcTarget.Others, x, y, z, name, k, prevCreatedYPos);
    }
