...
public class MapCreator : MonoBehaviour
{
    ...
    public void Create()
    {
        ...
        for (int i = 0; i < mapY; i++)
        {
            for (int j = 0; j < mapX; j++)
            {
                ...
                {
                    pv.RPC("CreateStone_Master", RpcTarget.MasterClient, x, y, z, obStonePrefab.name);
                }
                else if (mapInfo[i][j] == (int)MapInfo.Type.OBSTACLE_TREE)
                {
                    pv.RPC("CreateObject_Master", RpcTarget.MasterClient, x, y, z, obTreePrefab.name);
                }
                ...
                {
                    pv.RPC("CreateObject_Master", RpcTarget.MasterClient, x, y, z, storagePrefab.name);
                }
                else if (mapInfo[i][j] == (int)MapInfo.Type.ENGINE)
                {
                    Debug.Log("엔진생성 호출");
                    pv.RPC("CreateObject_Master", RpcTarget.MasterClient, x, y, z, enginePrefab.name);
                }
                else if (mapInfo[i][j] == (int)MapInfo.Type.Ax)
                {
                    Debug.Log("Ax 생성 호출");
                    pv.RPC("CreateObject_Master", RpcTarget.MasterClient, x, y, z, axPrefab.name);
                }
                else if (mapInfo[i][j] == (int)MapInfo.Type.Pick)
                {
                    pv.RPC("CreateObject_Master", RpcTarget.MasterClient, x, y, z, pickPrefab.name);
                }
                x++;
            }
            x = originX;
    ...
    void CreateObject_Master(float x, float y, float z,string name)
    {
        ...
        pv.RPC("CreateObject_Others", RpcTarget.Others, x, y, z, name);
    }

    [PunRPC]
    void CreateObject_Others(float x, float y, float z, string name)
    {
        Debug.Log("마스터 호출");
        GameObject _obj = PhotonNetwork.Instantiate(name, new Vector3(x, planeObject.transform.position.y + 0.5f, z), Quaternion.identity);
        _obj.transform.localScale = new Vector3(objScale * 10, objScale * 10, objScale * 10);
    ...
    [PunRPC]
    void CreateStone_Master(float x, float y, float z, string name)
    {
        float prevCreatedYPos = 0;
        int yCount = Random.Range(1, 5);
        for (int k = 0; k < yCount; k++)
        {
            GameObject _obj = PhotonNetwork.Instantiate(name, new Vector3(x * objScale * 10, prevCreatedYPos + objScale * 5, z * objScale * 10), Quaternion.identity);
            _obj.transform.localScale = new Vector3(objScale * 10, objScale * 10, objScale * 10);
            pv.RPC("CreateStone_Others", RpcTarget.Others, x, y, z, obStonePrefab.name, prevCreatedYPos);
    ...
    [PunRPC]
    void CreateStone_Others(float x, float y, float z, string name,float prevCreatedYPos)
    {
        GameObject _obj = PhotonNetwork.Instantiate(name, new Vector3(x * objScale * 10, prevCreatedYPos + objScale * 5, z * objScale * 10), Quaternion.identity);
        _obj.transform.localScale = new Vector3(objScale * 10, objScale * 10, objScale * 10);
    }
    ...
    [PunRPC]
    void CreateObject_Others(float x, float y, float z, string name, int k, float prevCreatedYPos)
    {
        Debug.Log("마스터 호출");
        GameObject _obj = PhotonNetwork.Instantiate(name, new Vector3(x * objScale * 10, prevCreatedYPos + objScale * 5, z * objScale * 10), Quaternion.identity);
        _obj.transform.localScale = new Vector3(objScale * 10, objScale * 10, objScale * 10);
