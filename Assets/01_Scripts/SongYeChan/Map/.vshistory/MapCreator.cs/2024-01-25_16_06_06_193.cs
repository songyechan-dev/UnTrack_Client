...
public class MapCreator : MonoBehaviour
{
    ...
    public void Create()
    {
        ...
        for (int i = 0; i < mapY; i++)
        {
            for (int j = 0; j < mapX; j++)
            {
                ...
                {
                    float prevCreatedYPos = 0;
                    int yCount = Random.Range(1, 5);
                    for (int k = 0; k < yCount; k++)
                    {
                        CreateObject(x, y, z,obStonePrefab.name,obStoneObject,k, ref prevCreatedYPos);
                        
                    }
                }
                else if (mapInfo[i][j] == (int)MapInfo.Type.OBSTACLE_TREE)
                {
                    pv.RPC("CreateObject_Master", RpcTarget.MasterClient, x, y, z, obTreePrefab.name, obTreeObject);
                }
                else if (mapInfo[i][j] == (int)MapInfo.Type.TRACK || mapInfo[i][j] == (int)MapInfo.Type.FiNISH_TRACK)
                {
                    pv.RPC("CreateTrack_Master", RpcTarget.MasterClient, x, y, z, trackPrefab.name, i, j);
                }
                ...
                {
                    pv.RPC("CreateObject_Master", RpcTarget.MasterClient, x, y, z, storagePrefab.name, storageObject);
                }
                else if (mapInfo[i][j] == (int)MapInfo.Type.ENGINE)
                {
                    Debug.Log("엔진생성 호출");
                    pv.RPC("CreateObject_Master", RpcTarget.MasterClient, x, y, z, enginePrefab.name, engineObject);
                }
                else if (mapInfo[i][j] == (int)MapInfo.Type.Ax)
                {
                    Debug.Log("Ax 생성 호출");
                    pv.RPC("CreateObject_Master", RpcTarget.MasterClient, x, y, z, axPrefab.name, axObject);
                }
                else if (mapInfo[i][j] == (int)MapInfo.Type.Pick)
                {
                    pv.RPC("CreateObject_Master", RpcTarget.MasterClient, x, y, z, pickPrefab.name,pickObject);
                }
                x++;
            }
            x = originX;
    ...
    void CreateObject(float x, float y, float z, string name, GameObject _obj, int k,ref float prevCreatedYPos)
    {
