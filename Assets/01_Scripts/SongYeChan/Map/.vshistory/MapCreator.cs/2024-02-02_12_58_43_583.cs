...
public class MapCreator : MonoBehaviour
{
    ...
    public List<(Vector3, Vector3, Vector3, Vector3)> toBeCreatedPos = new List<(Vector3, Vector3, Vector3, Vector3)>();
                ...
                if (planeList.ContainsKey(i))
                {
                    for (int j = 0; j < planeList[i].Count; j++)
                    {
                        if (planeList[i].Count > 1 && i + 1 < mapHeight && planeList.ContainsKey(i + 1) && planeList[i + 1].Count > 1)
                        {
                            ...
                            {
                                Vector3 first = new Vector3(i,0,j);
                                Vector3 secound = new Vector3(i,0,j+1);
                                Vector3 third = new Vector3(i + 1, 0, j);
                                Vector3 forth = new Vector3(i + 1, 0, j+1);
                                toBeCreatedPos.Add((first, secound, third, forth));
                            }
                        }
                    }
                }
