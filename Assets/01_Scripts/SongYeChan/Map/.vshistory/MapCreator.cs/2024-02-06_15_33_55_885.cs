    ...
    public void Create()
    {
        ...
        for (int i = 0; i < mapY; i++)
        {
            for (int j = 0; j < mapX; j++)
            {
                ...
                {
                    if ((j >= MapInfo.defaultStartTrackX -2 && j <= MapInfo.defaultEndTrackX +2) && (i < MapInfo.defaultStartTrackZ +1f))
                    {
                        //pv.RPC("CreatePlane_Master", RpcTarget.MasterClient, x, y, z)
                    }
                    else
                ...
                {
                    if ((j >= MapInfo.startPosition.x-2 && j < MapInfo.defaultEndTrackX +2) && (i < MapInfo.defaultStartTrackZ +1f))
                    {
