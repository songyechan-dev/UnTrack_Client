...
public class MapCreator : MonoBehaviour
{
    ...
    public GameObject startMinePrefab;
    [Header("")]
    ...
    private void MapDataCreate()
    {
        ...
        for (int y = 0; y < mapHeight; y++)
        {
            for (int x = 0; x < mapWidth; x++)
            {
                ...
                //출발TrackObject
                if ((x >= MapInfo.defaultStartTrackX && x <= MapInfo.defaultStartTrackX +4) && (y > MapInfo.defaultStartTrackZ && y <= MapInfo.defaultStartTrackZ +4))
                {
    ...
    public void Create()
    {
        ...
        int createdFactoryCount = 0;
        bool nextYCreateStartMine = false;
        for (int i = 0; i < mapY; i++)
        {
            for (int j = 0; j < mapX; j++)
            {
                ...
                else if (mapInfo[i][j] == (int)MapInfo.Type.START_MINE && nextYCreateStartMine)
                {
                    CreateObject_Master(x, y, z, startMinePrefab.name);
                }
