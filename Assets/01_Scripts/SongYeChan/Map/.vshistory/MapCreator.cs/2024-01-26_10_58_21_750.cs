            ...
            for (int j = 0; j < mapX; j++)
            {
                ...
                if (mapInfo[i][j] == (int)MapInfo.Type.OBSTACLE_STONE)
                {
                    if (mapInfo[i][j] == (int)MapInfo.Type.OBSTACLE_STONE)
                    {
                        ...
                        {
                            GameObject _obj = PhotonNetwork.Instantiate(obStonePrefab.name, new Vector3(x * objScale * 10, planeObject.transform.position.y + k, z * objScale * 10), Quaternion.identity);
