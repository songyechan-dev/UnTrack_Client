...
public class MapCreator : MonoBehaviour
{
    ...
    Dictionary<int,int> planeList = new Dictionary<int,int>();
    Dictionary<int, int> treeList = new Dictionary<int, int>();
    Dictionary<int, int> stoneList = new Dictionary<int, int>();
    ...
    public void CreateOB()
    {
        ...
        if (stoneList.Count > treeList.Count)
        {
            ...
            {
                if (!tempYPlaneList.ContainsKey(int.Parse(planeList[i].Split(",")[0])))
                {
                    tempYPlaneList.Add(int.Parse(planeList[i].Split(",")[0]), 0);
                }
            }
        }
        else
