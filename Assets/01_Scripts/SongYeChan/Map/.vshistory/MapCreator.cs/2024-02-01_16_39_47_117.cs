    ...
    private void MapDataCreate()
    {
        ...
        for (int y = 0; y < mapHeight; y++)
        {
            for (int x = 0; x < mapWidth; x++)
            {
                ...
                if ((x >= MapInfo.finishStartTrackX - 2 && x <= MapInfo.finishStartTrackX + 5) && (y > MapInfo.finishEndTrackX && y <= MapInfo.finishEndTrackZ + 5))
                {
    ...
    public void Create()
    {
        ...
        for (int i = 0; i < mapY; i++)
        {
            for (int j = 0; j < mapX; j++)
            {
                ...
                {
                    if (j == MapInfo.finishStartTrackX + 1 && i == MapInfo.finishEndTrackZ + 3)
                    {
                        CreateObject_Master(x, y, z, minePrefab.name);
                    }
                }
