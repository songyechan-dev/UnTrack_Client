...
public class MapCreator : MonoBehaviour
{
        ...
        for (int i = 0; i < mapY; i++)
        {
            for (int j = 0; j < mapX; j++)
            {
                ...
                if (mapInfo[i][j] == (int)MapInfo.Type.OBSTACLE_STONE)
                {
                    ...
                    {
                        CreateObject(x, y, z,obStonePrefab.name,obStoneObject,k, out prevCreatedYPos);
    ...
    {
        Debug.Log("마스터 호출");
        _obj = PhotonNetwork.Instantiate(name, new Vector3(x * objScale * 10, planeObject.transform.position.y + objScale * 5, z * objScale * 10), Quaternion.identity);
