    ...
    private void MapDataCreate()
    {
        ...
        for (int y = 0; y < mapHeight; y++)
        {
            for (int x = 0; x < mapWidth; x++)
            {
                ...
                //출발TrackObject
                if ((x >= MapInfo.defaultStartTrackX && x <= MapInfo.defaultStartTrackX +4) && (y > MapInfo.defaultStartTrackZ && y <= MapInfo.defaultStartTrackZ +4) && !isCreatedStartMine)
                {
                    mapInfo = ((int)(MapInfo.Type.START_MINE)).ToString();
                    isCreatedStartMine = true;
                }
