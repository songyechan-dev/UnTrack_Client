...
public class MapCreator : MonoBehaviour
{
    ...
    public void Create()
    {
        ...
        for (int i = 0; i < mapY; i++)
        {
            for (int j = 0; j < mapX; j++)
            {
                ...
                {
                    CreateObject(x, y, z, storagePrefab.name, storageObject);
                }
                else if (mapInfo[i][j] == (int)MapInfo.Type.ENGINE)
                {
                    Debug.Log("엔진생성 호출");
                    CreateObject(x, y, z, enginePrefab.name, engineObject);
                }
                else if (mapInfo[i][j] == (int)MapInfo.Type.Ax)
                {
                    Debug.Log("Ax 생성 호출");
                    CreateObject(x, y, z, axPrefab.name, axObject);
                }
                else if (mapInfo[i][j] == (int)MapInfo.Type.Pick)
                {
                    CreateObject(x, y, z, pickPrefab.name, pickObject);
                }
                x++;
            }
            x = originX;
    ...
    void CreateObject_Master(float x, float y, float z,string name,GameObject _obj)
    {
        ...
        pv.RPC("CreateObject_Otehrs", RpcTarget.Others, x, y, z, name, _obj);
    }

    [PunRPC]
    void CreateObject_Otehrs(float x, float y, float z, string name, GameObject _obj)
    {
        Debug.Log("마스터 호출");
        _obj = PhotonNetwork.Instantiate(name, new Vector3(x, planeObject.transform.position.y + 0.5f, z), Quaternion.identity);
        _obj.transform.localScale = new Vector3(objScale * 10, objScale * 10, objScale * 10);
        pv.RPC(C, RpcTarget.Others,)
    }
