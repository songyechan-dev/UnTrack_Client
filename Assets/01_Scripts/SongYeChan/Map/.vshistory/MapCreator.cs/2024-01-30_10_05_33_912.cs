...
public class MapCreator : MonoBehaviour
{
    ...
    void CreateTrack_Master(float x, float y, float z, string name,int i, int j)
    {
        ...
        {
            trackObject.transform.localEulerAngles = new Vector3(trackPrefab.transform.rotation.x, MapInfo.startTrackYRotation, trackPrefab.transform.localRotation.z);
            trackManager.finalTrack = trackObject;
            isTrack = true;
        }
        else
        {
            trackObject.transform.localEulerAngles = new Vector3(trackPrefab.transform.rotation.x, MapInfo.endTrackYRotation, trackPrefab.transform.localRotation.z);
    ...
    void CreateTrack_Others(float x, float y, float z, int ViewID, int i, int j,bool isTrack)
    {
        ...
        trackObject.AddComponent<TrackInfo>();
        trackObject.GetComponent<TrackInfo>().SetMyDirection(TrackInfo.MyDirection.FORWARD, new Vector3(trackPrefab.transform.rotation.x, trackPrefab.transform.rotation.y, trackPrefab.transform.localRotation.z));
        trackObject.GetComponent<TrackInfo>().isElectricityFlowing = true;
        trackObject.tag = "Track";
        MapInfo.trackYscale = trackPrefab.transform.localScale.y;
        trackObject.transform.localScale = new Vector3(objScale * 10, trackPrefab.transform.localScale.y, objScale * 10);
