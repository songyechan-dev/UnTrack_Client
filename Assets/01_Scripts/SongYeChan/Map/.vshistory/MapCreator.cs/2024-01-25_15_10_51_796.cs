...
public class MapCreator : MonoBehaviour
{
        ...
        for (int i = 0; i < mapY; i++)
        {
            for (int j = 0; j < mapX; j++)
            {
                ...
                if (mapInfo[i][j] == (int)MapInfo.Type.OBSTACLE_STONE)
                {
                    ...
                    {
                        pv.RPC("CreateObject_Master", RpcTarget.Others, x, y, z, name, obTreeObject,k,prevCreatedYPos);

                    }
                }
                else if (mapInfo[i][j] == (int)MapInfo.Type.OBSTACLE_TREE)
                {
                    pv.RPC("CreateObject_Master", RpcTarget.Others, x, y, z, name, obTreeObject);
                }
                else if (mapInfo[i][j] == (int)MapInfo.Type.TRACK || mapInfo[i][j] == (int)MapInfo.Type.FiNISH_TRACK)
                {
                    pv.RPC("CreateTrack_Master", RpcTarget.Others, x, y, z, name, i, j);
                }
                else if (mapInfo[i][j] == (int)MapInfo.Type.FACTORY)
                {
                    if (round == 1)
                    {
                        pv.RPC("CreateFactory_Master", RpcTarget.MasterClient, x, y, z, productionMachinePrefab.name, FACTORYTYPE.ProductionMachine);
                    }
                    else
                    {
                        ...
                        {
                            pv.RPC("CreateFactory_Master", RpcTarget.MasterClient, x, y, z, productionMachinePrefab.name, FACTORYTYPE.WaterTank);
                        }
                        else if (StateManager.Instance().dynamiteMachines.Count > 0)
                        {
                            pv.RPC("CreateFactory_Master", RpcTarget.MasterClient, x, y, z, productionMachinePrefab.name, FACTORYTYPE.DynamiteMachine);
                        }
                    }
                }
                else if (mapInfo[i][j] == (int)MapInfo.Type.STORAGE)
                {
                    pv.RPC("CreateObject_Master", RpcTarget.Others, x, y, z, name, storageObject);
                }
                else if (mapInfo[i][j] == (int)MapInfo.Type.ENGINE)
                {
                    Debug.Log("엔진생성 호출");
                    pv.RPC("CreateObject_Master", RpcTarget.Others, x, y, z, name, engineObject);
                }
                else if (mapInfo[i][j] == (int)MapInfo.Type.Ax)
                {
                    Debug.Log("Ax 생성 호출");
                    pv.RPC("CreateObject_Master", RpcTarget.Others, x, y, z, name, axObject);
                }
                else if (mapInfo[i][j] == (int)MapInfo.Type.Pick)
                {
                    pv.RPC("CreateObject_Master", RpcTarget.Others, x, y, z, name, pickObject);
                }
                x++;
            }
            x = originX;
    ...
    [PunRPC]
    void CreateObject_Master(float x, float y, float z,string name,GameObject _obj)
    {
        Debug.Log("마스터 호출");
        _obj = PhotonNetwork.Instantiate(name, new Vector3(x, planeObject.transform.position.y + 0.5f, z), Quaternion.identity);
        _obj.transform.localScale = new Vector3(objScale * 10, objScale * 10, objScale * 10);
        pv.RPC("CreateObject_Others", RpcTarget.Others, x, y, z, name, _obj);
    }

    [PunRPC]
    void CreateObject_Others(float x, float y, float z, string name, GameObject _obj)
    {
        Debug.Log("마스터 호출");
        _obj = PhotonNetwork.Instantiate(name, new Vector3(x, planeObject.transform.position.y + 0.5f, z), Quaternion.identity);
        _obj.transform.localScale = new Vector3(objScale * 10, objScale * 10, objScale * 10);
    ...
    [PunRPC]
    void CreateObject_Master(float x, float y, float z, string name, GameObject _obj, int k,float prevCreatedYPos)
    {
        _obj = PhotonNetwork.Instantiate(name, new Vector3(x * objScale * 10, prevCreatedYPos + objScale * 5, z * objScale * 10), Quaternion.identity);
        _obj.transform.localScale = new Vector3(objScale * 10, objScale * 10, objScale * 10);
        prevCreatedYPos = _obj.transform.position.y;
        pv.RPC("CreateObject_Others", RpcTarget.Others, x, y, z, name, obTreeObject, k, prevCreatedYPos);
    }

    [PunRPC]
    void CreateObject_Others(float x, float y, float z, string name, GameObject _obj, int k, float prevCreatedYPos)
    {
        _obj = PhotonNetwork.Instantiate(name, new Vector3(x * objScale * 10, prevCreatedYPos + objScale * 5, z * objScale * 10), Quaternion.identity);
        _obj.transform.localScale = new Vector3(objScale * 10, objScale * 10, objScale * 10);
        prevCreatedYPos = _obj.transform.position.y;
    ...
    {
        trackObject = PhotonNetwork.Instantiate(name, new Vector3(x * objScale * 10, trackPrefab.transform.localScale.y / 2, z * objScale * 10),Quaternion.identity);
        ...
        pv.RPC("CreateTrack_Others", RpcTarget.Others, x, y, z, name, i,j);
    }
    [PunRPC]
    void CreateTrack_Others(float x, float y, float z, string name, int i, int j)
    {
        trackObject = PhotonNetwork.Instantiate(name, new Vector3(x * objScale * 10, trackPrefab.transform.localScale.y / 2, z * objScale * 10), Quaternion.identity);
        trackObject.AddComponent<TrackInfo>();
        trackObject.GetComponent<TrackInfo>().SetMyDirection(TrackInfo.MyDirection.FORWARD, new Vector3(0, 0, 0));
