...
public class MapCreator : MonoBehaviour
{
    ...
    void CreateTrack_Master(float x, float y, float z, string name,int i, int j)
    {
        ...
        trackObject = PhotonNetwork.Instantiate(name, new Vector3(x * objScale * 10, (trackPrefab.transform.localScale.y *0.05f) / 2, z * objScale * 10),Quaternion.Euler(new Vector3(0,MapInfo.startTrackYRotation,0)));
        ...
        {
            trackObject.transform.localEulerAngles = new Vector3(trackDefaultXRotation, MapInfo.startTrackYRotation, trackDefaultZRotation);
            trackManager.finalTrack = trackObject;
            isTrack = true;
        }
        else
        {
            trackObject.transform.localEulerAngles = new Vector3(trackDefaultXRotation, MapInfo.endTrackYRotation, trackDefaultZRotation);
    ...
    void CreateTrack_Others(float x, float y, float z, int ViewID, int i, int j,bool isTrack)
    {
        ...
        trackObject.AddComponent<TrackInfo>();
        trackObject.GetComponent<TrackInfo>().SetMyDirection(TrackInfo.MyDirection.FORWARD, new Vector3(trackDefaultXRotation, trackPrefab.transform.rotation.y, trackDefaultZRotation));
        trackObject.GetComponent<TrackInfo>().isElectricityFlowing = true;
        trackObject.tag = "Track";
        MapInfo.trackYscale = trackPrefab.transform.localScale.y;
        trackObject.transform.localScale = new Vector3(objScale * 10, trackPrefab.transform.localScale.y, objScale * 10);
