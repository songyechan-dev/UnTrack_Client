        ...
        for (int i = 0; i < mapY; i++)
        {
            for (int j = 0; j < mapX; j++)
            {
                ...
                if (mapInfo[i][j] == (int)MapInfo.Type.OBSTACLE_STONE)
                {
                    ...
                    {
                        CreateObject(x, y, z,obStonePrefab.name,obStoneObject,k);
                        prevCreatedYPos = obStoneObject.transform.position.y;
                    }
                }
                else if (mapInfo[i][j] == (int)MapInfo.Type.OBSTACLE_TREE)
                {
                    CreateObject( x, y, z, obTreePrefab.name, obTreeObject,0);
                }
                ...
                {
                    CreateObject(x, y, z, storagePrefab.name, storageObject,0);
                }
                else if (mapInfo[i][j] == (int)MapInfo.Type.ENGINE)
                {
                    Debug.Log("엔진생성 호출");
                    CreateObject(x, y, z, enginePrefab.name, engineObject,0);
                }
                else if (mapInfo[i][j] == (int)MapInfo.Type.Ax)
                {
                    Debug.Log("Ax 생성 호출");
                    CreateObject(x, y, z, axPrefab.name, axObject,0);
                }
                else if (mapInfo[i][j] == (int)MapInfo.Type.Pick)
                {
                    CreateObject(x, y, z, pickPrefab.name, pickObject,0);
                }
                x++;
            }
            x = originX;
