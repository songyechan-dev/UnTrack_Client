...
public class MapCreator : MonoBehaviour
{
    ...
    public void Create()
    {
        ...
        for (int i = 0; i < mapY; i++)
        {
            for (int j = 0; j < mapX; j++)
            {
                ...
                {
                    pv.RPC("CreateObject_Master", RpcTarget.MasterClient, x, y, z, obTreePrefab, obTreeObject);
                }
                ...
                {
                    pv.RPC("CreateObject_Master", RpcTarget.MasterClient, x, y, z, storagePrefab, storageObject);

                }
                else if (mapInfo[i][j] == (int)MapInfo.Type.ENGINE)
                {
                    Debug.Log("엔진생성 호출");
                    pv.RPC("CreateObject_Master", RpcTarget.MasterClient, x, y, z, enginePrefab, engineObject);
                }
                else if (mapInfo[i][j] == (int)MapInfo.Type.Ax)
                {
                    Debug.Log("Ax 생성 호출");
                    pv.RPC("CreateObject_Master", RpcTarget.MasterClient, x, y, z, axPrefab, axObject);
                }
                else if (mapInfo[i][j] == (int)MapInfo.Type.Pick)
                {
                    pv.RPC("CreateObject_Master", RpcTarget.MasterClient, x, y, z, pickPrefab, pickObject);
                }
                x++;
            }
            x = originX;
    ...
    [PunRPC]
    void CreateObject_Master(float x, float y, float z,GameObject _prefab,GameObject _obj,int k = 0)
    {
        Debug.Log("마스터 호출");
    ...
    [PunRPC]
    void CreateObject_Others(float x, float y, float z, GameObject _prefab, GameObject _obj, int k = 0)
    {
        _obj = Instantiate(obStonePrefab);
        _obj.transform.position = new Vector3(x * objScale * 10, k == 0 ? planeObject.transform.position.y + objScale * 5 : (prevCreatedYPos + objScale * 10), z * objScale * 10);
        _obj.transform.localScale = new Vector3(objScale * 10, objScale * 10, objScale * 10);
