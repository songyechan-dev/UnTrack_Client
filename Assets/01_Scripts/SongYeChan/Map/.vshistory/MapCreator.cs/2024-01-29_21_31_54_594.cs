...
public class MapCreator : MonoBehaviour
{
            ...
            for (int j = 0; j < mapX; j++)
            {
                ...
                if (mapInfo[i][j] == (int)MapInfo.Type.OBSTACLE_STONE)
                {
                    if (mapInfo[i][j] == (int)MapInfo.Type.OBSTACLE_STONE)
                    {
                        ...
                        {
                            GameObject _obj = PhotonNetwork.Instantiate(obStonePrefab.name, new Vector3(x * objScale * 10, (k == 1 ? planeObject.transform.position.y + 0.5f : prevCreatedYPos + 1), z * objScale * 10), trackPrefab.transform.rotation);
    ...
    void CreateTrack_Master(float x, float y, float z, string name,int i, int j)
    {
        ...
        trackObject = PhotonNetwork.Instantiate(name, new Vector3(x * objScale * 10, trackPrefab.transform.localScale.y / 2, z * objScale * 10),Quaternion.Euler(new Vector3(trackPrefab.transform.rotation.x,MapInfo.startTrackYRotation, trackPrefab.transform.rotation.zx)));
        trackObject.AddComponent<TrackInfo>();
        trackObject.GetComponent<TrackInfo>().SetMyDirection(TrackInfo.MyDirection.FORWARD, new Vector3(trackPrefab.transform.rotation.x, 0, trackPrefab.transform.rotation.z));
        ...
        {
            trackObject.transform.localEulerAngles = new Vector3(trackPrefab.transform.rotation.x, MapInfo.startTrackYRotation, trackPrefab.transform.rotation.z);
            trackManager.finalTrack = trackObject;
            isTrack = true;
        }
        else
        {
            trackObject.transform.localEulerAngles = new Vector3(trackPrefab.transform.rotation.x, MapInfo.endTrackYRotation, trackPrefab.transform.rotation.z);
