...
public class MapCreator : MonoBehaviour
{
    ...
    public void CreateOB()
    {
        ...
        if (treeCount > stoneCount)
        {
            ...
            RaycastHit hit;
            Ray rayV1 = new Ray(new Vector3(toBeCreatedPos[rand].v1.x, toBeCreatedPos[rand].v1.y, toBeCreatedPos[rand].v1.z), Vector3.up);
            Ray rayV2 = new Ray(new Vector3(toBeCreatedPos[rand].v2.x, toBeCreatedPos[rand].v2.y, toBeCreatedPos[rand].v2.z), Vector3.up);
            Ray rayV3 = new Ray(new Vector3(toBeCreatedPos[rand].v3.x, toBeCreatedPos[rand].v3.y, toBeCreatedPos[rand].v3.z), Vector3.up);
            Ray rayV4 = new Ray(new Vector3(toBeCreatedPos[rand].v4.x, toBeCreatedPos[rand].v4.y, toBeCreatedPos[rand].v4.z), Vector3.up);

            //생성전 오브젝트들이 생성이 되어있는지 확인
            if (Physics.Raycast(rayV1, out hit))
            {
                CreateOB();
            }
            else if (Physics.Raycast(rayV2, out hit))
            {
                CreateOB();
            }
            else if (Physics.Raycast(rayV3, out hit))
            {
                CreateOB();
            }
            else if (Physics.Raycast(rayV4, out hit))
            {
                CreateOB();
            }
