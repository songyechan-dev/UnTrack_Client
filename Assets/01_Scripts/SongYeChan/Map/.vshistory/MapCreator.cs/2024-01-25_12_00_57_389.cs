...
public class MapCreator : MonoBehaviour
{
        ...
        for (int i = 0; i < mapY; i++)
        {
            for (int j = 0; j < mapX; j++)
            {
                ...
                if (mapInfo[i][j] == (int)MapInfo.Type.OBSTACLE_STONE)
                {
                    ...
                    {
                        pv.RPC("CreateObject_Master", RpcTarget.MasterClient, x, y, z,obStonePrefab);
    ...
    [PunRPC]
    void CreateObject_Master(float x, float y, float z,GameObject _prefab,GameObject _obj)
    {
