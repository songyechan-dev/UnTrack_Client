...
public class MapCreator : MonoBehaviour
{
    ...
    public void CreateOB()
    {
        ...
        {
            Dictionary<int,int> tempPlaneList = new Dictionary<int,int>();
            for (int i = 0; i < planeList.Count; i++)
            {
                if (!tempPlaneList.ContainsKey(int.Parse(planeList[i].Split(",")[0])))
                {
                    tempPlaneList.Add(int.Parse(planeList[i].Split(",")[0]), 0);
                }
            }
        }
        else
