    ...
    public void Create()
    {
        ...
        for (int i = 0; i < mapY; i++)
        {
            for (int j = 0; j < mapX; j++)
            {
                ...
                {
                    pv.RPC("CreateObject_Others", RpcTarget.MasterClient, x, y, z, obTreePrefab, obTreeObject);
                }
                ...
                {
                    pv.RPC("CreateObject_Others", RpcTarget.MasterClient, x, y, z, storagePrefab, storageObject);

                }
                else if (mapInfo[i][j] == (int)MapInfo.Type.ENGINE)
                {
                    Debug.Log("엔진생성 호출");
                    pv.RPC("CreateObject_Others", RpcTarget.MasterClient, x, y, z, enginePrefab, engineObject);
                }
                else if (mapInfo[i][j] == (int)MapInfo.Type.Ax)
                {
                    Debug.Log("Ax 생성 호출");
                    pv.RPC("CreateObject_Others", RpcTarget.MasterClient, x, y, z, axPrefab, axObject);
                }
                else if (mapInfo[i][j] == (int)MapInfo.Type.Pick)
                {
                    pv.RPC("CreateObject_Others", RpcTarget.MasterClient, x, y, z, pickPrefab, pickObject);
                }
                x++;
            }
            x = originX;
