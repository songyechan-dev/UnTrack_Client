...
public class MapCreator : MonoBehaviour
{
    ...
    {
        Debug.Log("마스터 호출");
        _obj = PhotonNetwork.Instantiate(name, new Vector3(x * objScale * 10, planeObject.transform.position.y - (planeObject.transform.localScale.y /2) + objScale * 2.5f, z * objScale * 10), Quaternion.identity);
    ...
    void CreateObject(float x, float y, float z, string name, GameObject _obj, int k,ref float prevCreatedYPos)
    {
        ...
        Debug.Log("마스터 호출");
        _obj = PhotonNetwork.Instantiate(name, new Vector3(x * objScale * 10, prevCreatedYPos + objScale * 10, z * objScale * 10), Quaternion.identity);
        _obj.transform.localScale = new Vector3(objScale * 10, objScale * 10, objScale * 10);
