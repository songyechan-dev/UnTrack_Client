...
public class MapCreator : MonoBehaviour
{
        ...
        for (int i = 0; i < mapY; i++)
        {
            for (int j = 0; j < mapX; j++)
            {
                ...
                if (mapInfo[i][j] == (int)MapInfo.Type.OBSTACLE_STONE)
                {
                    ...
                    {
                        pv.RPC("CreateObject_Master", RpcTarget.MasterClient, x, y, z,obStonePrefab,obStoneObject,k);
                        prevCreatedYPos = obStoneObject.transform.position.y;
                    }
                }
                else if (mapInfo[i][j] == (int)MapInfo.Type.OBSTACLE_TREE)
                {
                    pv.RPC("CreateObject_Master", RpcTarget.MasterClient, x, y, z, obTreePrefab, obTreeObject,0);
                }
                ...
                {
                    pv.RPC("CreateObject_Master", RpcTarget.MasterClient, x, y, z, storagePrefab, storageObject,0);

                }
                else if (mapInfo[i][j] == (int)MapInfo.Type.ENGINE)
                {
                    Debug.Log("엔진생성 호출");
                    pv.RPC("CreateObject_Master", RpcTarget.MasterClient, x, y, z, enginePrefab, engineObject,0);
                }
                else if (mapInfo[i][j] == (int)MapInfo.Type.Ax)
                {
                    Debug.Log("Ax 생성 호출");
                    pv.RPC("CreateObject_Master", RpcTarget.MasterClient, x, y, z, axPrefab, axObject,0);
                }
                else if (mapInfo[i][j] == (int)MapInfo.Type.Pick)
                {
                    pv.RPC("CreateObject_Master", RpcTarget.MasterClient, x, y, z, pickPrefab, pickObject,0);
                }
                x++;
            }
            x = originX;
    ...
    void CreatePlane_Master(float x, float y, float z)
    {
        ...
        pv.RPC("CreatePlane_Others", RpcTarget.Other, x, y, z);
    }
    ...
    [PunRPC]
    void CreateObject_Master(float x, float y, float z,GameObject _prefab,GameObject _obj,int k)
    {
        Debug.Log("마스터 호출");
        _obj = Instantiate(obStonePrefab);
        _obj.transform.position = new Vector3(x * objScale * 10, k == 0 ? planeObject.transform.position.y + objScale * 5 : (prevCreatedYPos + objScale * 10), z * objScale * 10);
        _obj.transform.localScale = new Vector3(objScale * 10, objScale * 10, objScale * 10);
        pv.RPC("CreateObject_Others", RpcTarget.Other, x, y, z, obStonePrefab, obStoneObject, k);
    }

    [PunRPC]
    void CreateObject_Others(float x, float y, float z, GameObject _prefab, GameObject _obj, int k)
    {
        _obj = Instantiate(obStonePrefab);
        _obj.transform.position = new Vector3(x * objScale * 10, k == 0 ? planeObject.transform.position.y + objScale * 5 : (prevCreatedYPos + objScale * 10), z * objScale * 10);
        _obj.transform.localScale = new Vector3(objScale * 10, objScale * 10, objScale * 10);
