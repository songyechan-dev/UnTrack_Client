...
public class TrackInfo : MonoBehaviour
{
    ...
    public void SetElectricityFlowing(bool _isElectricityFlowing)
    {
        ...
        {
            if (Physics.Raycast(new Ray(transform.position, Vector3.left), out leftHit, maxDistance))
            {
                if (leftHit.transform.CompareTag(trackTagName))
                {
                    if (!leftHit.transform.GetComponent<TrackInfo>().isElectricityFlowing)
                    {
                        Debug.Log("꺼져있다");
                    }
                }
            }
            if (Physics.Raycast(new Ray(transform.position, Vector3.left), out rightHit, maxDistance))
            {
                if (rightHit.transform.CompareTag(trackTagName))
                {
                    if (!rightHit.transform.GetComponent<TrackInfo>().isElectricityFlowing)
                    {
