...
public class TrackInfo : MonoBehaviour
{
    ...
    public void SetElectricityFlowing(bool _isElectricityFlowing)
    {
        RaycastHit upHit;
        RaycastHit downHit;
        if (_isElectricityFlowing)
        {
            if (Physics.Raycast(new Ray(transform.position, Vector3.up), out upHit, maxDistance))
            {
                if (upHit.transform.CompareTag(trackTagName))
                {
                    if (!upHit.transform.GetComponent<TrackInfo>().isElectricityFlowing)
                    {
                        Debug.Log("꺼져있다");
                    }
                }
            }
            if (Physics.Raycast(new Ray(transform.position, Vector3.down), out downHit, maxDistance))
            {
                if (downHit.transform.CompareTag(trackTagName))
                {
                    if (!downHit.transform.GetComponent<TrackInfo>().isElectricityFlowing)
                    {
                        Debug.Log("꺼져있다_RIght");
                    }
                }
            }
        }
