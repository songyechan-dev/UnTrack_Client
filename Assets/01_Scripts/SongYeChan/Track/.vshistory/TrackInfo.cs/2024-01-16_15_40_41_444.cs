...
public class TrackInfo : MonoBehaviour
{
    ...
    public void SetMyDirection(ShapeDirection _shapeDirection,Vector3 _myDirection)
    {
        switch (_shapeDirection)
        {
            case ShapeDirection.FORWARD:
                transform.localEulerAngles = _myDirection;
                shapeDirection = ShapeDirection.FORWARD;
                break;
            case ShapeDirection.BACK:
                transform.localEulerAngles = _myDirection;
                shapeDirection = ShapeDirection.BACK;
                break;
            case ShapeDirection.LEFT:
                transform.localEulerAngles = _myDirection;
                shapeDirection = ShapeDirection.LEFT;
                Debug.Log("변경됨 LEFT!");
                break;
            case ShapeDirection.RIGHT:
                transform.localEulerAngles = _myDirection;
                shapeDirection = ShapeDirection.RIGHT;
                Debug.Log("변경됨 RIGHT");
                break;
            default:
                break;
