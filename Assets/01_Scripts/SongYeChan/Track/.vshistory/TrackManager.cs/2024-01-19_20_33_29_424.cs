...
public class TrackManager : MonoBehaviour
{
    ...
    public bool isTrackConnectFailed = false;
    ...
    public GameObject trackChildPrefab_LeftAndRight;
    public GameObject trackChildPrefab;
            ...
            if (hit.transform.CompareTag(planeTagName))
            {
                ...
                if (Physics.Raycast(new Ray(track.transform.position, Vector3.left), out leftHit, maxDistance))
                {
                    if (leftHit.transform.CompareTag(trackTagName))
                    {
                        ...
                        GameObject tempChild = Instantiate(trackChildPrefab_LeftAndRight,leftTrackInfo.transform);
                        tempChild.transform.localEulerAngles = new Vector3(0, -90, 0);

                        _trackInfo.SetMyDirection(TrackInfo.MyDirection.FORWARD, new Vector3(0, 90f, 0));

                        GameObject trackChild = Instantiate(trackChildPrefab, _trackInfo.transform);
                        trackChild.transform.localEulerAngles = _trackInfo.transform.eulerAngles;
                ...
                if (Physics.Raycast(new Ray(track.transform.position, Vector3.right), out rightHit, maxDistance))
                {
                    if (rightHit.transform.CompareTag(trackTagName))
                    {
                        ...
                        GameObject tempChild = Instantiate(trackChildPrefab_LeftAndRight, rightTrackInfo.transform);
                        tempChild.transform.localEulerAngles = new Vector3(0, 0, 0);
                ...
                if (Physics.Raycast(new Ray(track.transform.position, Vector3.back), out backHit, maxDistance))
                {
                    if (backHit.transform.CompareTag(trackTagName))
                    {
                        ...
                        GameObject trackChild = Instantiate(trackChildPrefab, _trackInfo.transform);
                        tempChild.transform.localEulerAngles = _trackInfo.transform.eulerAngles;
                        _trackInfo.isElectricityFlowing = true;
                ...
                if (electricityFlowingList.Count >= 2)
                {
                    ...
                    if (count <= 0)
                    {
                        for (int i = 0; i < electricityFlowingList.Count; i++)
                        {
                            ...
                            Destroy(electricityFlowingList[i].GetChild(0).gameObject);
                            GameObject childObject = Instantiate(trackChildPrefab, electricityFlowingList[i].transform);
                        }
                        TrackConnectFailed(track);
                    }
    ...
    public void TrackConnectFailed(GameObject track)
    {
        isTrackConnectFailed = true;
