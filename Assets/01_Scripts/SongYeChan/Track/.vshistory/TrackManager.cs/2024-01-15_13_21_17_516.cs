    ...
    public void TrackCreate(Ray _ray)
    {
        ...
        if (Physics.Raycast(ray, out hit))
        {
            if (hit.transform.CompareTag(planeTagName))
            {
                ...
                if (leftHit.transform.GetComponent<TrackInfo>() != null && leftHit.transform != null && leftHit.transform.GetComponent<TrackInfo>().isElectricityFlowing)
                {
                    Debug.Log("왼쪽의 객체가 전기가 흐르고있음");
                }
                else if (rightHit.transform.GetComponent<TrackInfo>() != null && rightHit.transform != null && rightHit.transform.GetComponent<TrackInfo>().isElectricityFlowing)
                {
                    Debug.Log("오른쪽의 객체가 전기가 흐르고있음");
                }
