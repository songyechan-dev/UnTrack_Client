...
public class TrackManager : MonoBehaviourPun
{
    ...
    public void TrackCreate(Ray _ray)
    {
        ...
        if (Physics.Raycast(_ray, out hit))
        {
            ...
            GameObject track = PhotonNetwork.Instantiate("Track", new Vector3(Mathf.Round(_ray.origin.x), (trackPrefab.transform.localScale.y * 0.001f), Mathf.Round(_ray.origin.z)), trackPrefab.transform.rotation);
            ...
            if (Physics.Raycast(new Ray(track.transform.position, Vector3.left), out leftHit, maxDistance))
            {
                if (leftHit.transform.CompareTag(trackTagName))
                {
                    ...
                    leftTrackInfo.SetMyDirection(TrackInfo.MyDirection.RIGHT, new Vector3(MapInfo.trackDefaultXRotation, 90f, 0));
