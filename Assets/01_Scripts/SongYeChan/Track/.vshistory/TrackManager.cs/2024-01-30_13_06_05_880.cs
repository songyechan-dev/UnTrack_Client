...
public class TrackManager : MonoBehaviourPun
{
    ...
    public void TrackCreate(Ray _ray)
    {
        ...
        if (Physics.Raycast(_ray, out hit))
        {
            ...
            GameObject track = PhotonNetwork.Instantiate("Track", new Vector3(Mathf.Round(_ray.origin.x), hit.transform.localPosition.y + (trackPrefab.transform.localScale.y * 0.15f), Mathf.Round(_ray.origin.z)), trackPrefab.transform.rotation);
