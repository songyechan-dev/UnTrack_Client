...
public class TrackManager : MonoBehaviour
{
    ...
    private int electricityFlowingCount = 0;
    ...
    public void TrackCreate(Ray _ray)
    {
        ...
        if (Physics.Raycast(ray, out hit))
        {
            if (hit.transform.CompareTag(planeTagName))
            {
                ...
                {
                    if (leftHit.transform.GetComponent<TrackInfo>().isElectricityFlowing && rightHit.transform.GetComponent<TrackInfo>().isElectricityFlowing)
                    {
