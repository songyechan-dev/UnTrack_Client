...
public class TrackManager : MonoBehaviourPun
{
        ...
        if (Physics.Raycast(_ray, out hit))
        {
            ...
            if (Physics.Raycast(new Ray(track.transform.position, Vector3.left), out leftHit, maxDistance))
            {
                ...
                if (leftHit.transform.CompareTag(trackTagName))
                {
                    ...
                    object[] trackData = new object[] { _trackInfo.GetComponent<PhotonView>().ViewID,leftTrackInfo.GetComponent<PhotonView>().ViewID, (int)TrackInfo.MyDirection.RIGHT, (int)TrackInfo.MyDirection.FORWARD, new Vector3(MapInfo.trackDefaultXRotation, 90f, MapInfo.trackDefaultZRotation), new Vector3(MapInfo.trackDefaultXRotation, 90f, MapInfo.trackDefaultZRotation, prevMesh) };
    ...
    void SetAroundTrack(int _trackViewId, int _aroundTrackViewId, int _trackDirection, int _aroundTrackDirection, Vector3 _trackRotation, Vector3 _aroundTrackRotation,int meshDirection)
    {
