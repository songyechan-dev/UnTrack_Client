        ...
        if (Physics.Raycast(_ray, out hit))
        {
            ...
            if (Physics.Raycast(new Ray(track.transform.position, Vector3.forward), out forwardHit, maxDistance))
            {
                ...
                if (forwardHit.transform.CompareTag(trackTagName))
                {
                    ...
                    if (forwardTrackInfo.transform.localEulerAngles.y == 0 || forwardTrackInfo.transform.localEulerAngles.y == -180)
                    {
                        Debug.Log("여기 호출");
                    ...
                    else if (forwardTrackInfo.transform.localEulerAngles.y == 180)
                    {
                        Debug.Log("여기 호출");
