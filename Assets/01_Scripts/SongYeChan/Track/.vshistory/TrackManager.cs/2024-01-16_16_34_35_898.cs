...
public class TrackManager : MonoBehaviour
{
    ...
    public void TrackCreate(Ray _ray)
    {
        ...
        if (Physics.Raycast(ray, out hit))
        {
            if (hit.transform.CompareTag(planeTagName))
            {
                ...
                if ((leftHit.transform != null && !leftHit.transform.GetComponent<TrackInfo>().isElectricityFlowing && rightHit.transform == null && leftHit.transform != finalTrack)
    ...
    {
        if (finalTrack.Count > 0)
        {
            finalTrack.Dequeue();
        }
        finalTrack.Enqueue(gameObject);
    }
}
