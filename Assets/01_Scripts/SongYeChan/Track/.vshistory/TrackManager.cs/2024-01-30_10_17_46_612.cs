...
public class TrackManager : MonoBehaviourPun
{
    ...
    public void TrackCreate(Ray _ray)
    {
        ...
        if (Physics.Raycast(_ray, out hit))
        {
            ...
            float roundedX = Mathf.Round(_ray.origin.x);
            roundedX += (Mathf.Floor(_ray.origin.x + 0.5f) == roundedX) ? trackPrefab.transform.localScale.z : -trackPrefab.transform.localScale.z;
            float roundedZ = Mathf.Round(_ray.origin.z);
            roundedZ += (Mathf.Floor(_ray.origin.z + 0.5f) == roundedZ) ? trackPrefab.transform.localScale.z : -trackPrefab.transform.localScale.z;
            GameObject track = PhotonNetwork.Instantiate("Track", new Vector3(Mathf.Round(_ray.origin.x), 0 + (trackPrefab.transform.localScale.y / 2), Mathf.Round(_ray.origin.z)), trackPrefab.transform.rotation);
