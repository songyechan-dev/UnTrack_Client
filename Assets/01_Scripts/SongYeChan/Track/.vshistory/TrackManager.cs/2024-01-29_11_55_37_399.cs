...
public class TrackManager : MonoBehaviourPun
{
    ...
    void SetAroundTrack(int _trackViewId, int _aroundTrackViewId,TrackInfo.MyDirection _trackDirection, TrackInfo.MyDirection _aroundTrackDirection, Vector3 _trackRotation, Vector3 _aroundTrackRotation)
    {
        ...
        aroundTrackInfo.SetMyDirection(_aroundTrackDirection, _aroundTrackRotation);
        _trackInfo.SetMyDirection(_trackDirection, _trackRotation);
        ...
        if (Physics.Raycast(_ray, out hit))
        {
            ...
            if (Physics.Raycast(new Ray(track.transform.position, Vector3.left), out leftHit, maxDistance))
            {
                if (leftHit.transform.CompareTag(trackTagName))
                {
                    ...
                    object[] trackData = new object[] { _trackInfo.GetComponent<PhotonView>().ViewID,leftTrackInfo.GetComponent<PhotonView>().ViewID, TrackInfo.MyDirection.RIGHT, TrackInfo.MyDirection.FORWARD, new Vector3(0, 90f, 0), new Vector3(0, 90f, 0) };
    ...
    void OnEvent(EventData photonEvent)
    {
        ...
        if (photonEvent.Code == (int)SendDataInfo.Info.AROUND_TRACK_INFO)
        {
            ...
            int _aroundTrackViewId = (int)receivedData[0];
            int _trackViewID = (int)receivedData[0];
            int _trackViewID = (int)receivedData[0];
            int _trackViewID = (int)receivedData[0];
            int _trackViewID = (int)receivedData[0];
