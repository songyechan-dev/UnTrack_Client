        ...
        if (Physics.Raycast(_ray, out hit))
        {
            ...
            if (Physics.Raycast(new Ray(track.transform.position, Vector3.forward), out forwardHit, maxDistance))
            {
                ...
                if (forwardHit.transform.CompareTag(trackTagName))
                {
                    ...
                    if (forwardTrackInfo.transform.localEulerAngles.y == 0 || Mathf.Abs(forwardTrackInfo.transform.localEulerAngles.y) ==90f)
                    {
                    ...
                    else if (Mathf.Abs(forwardTrackInfo.transform.localEulerAngles.y) == 180)
                    {
