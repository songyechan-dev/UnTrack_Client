...
public class ObstacleManager : MonoBehaviour
{
    ...
    public void GenerateIngredient(PlayerController _player)
    {
        ...
        RaycastHit hit;
        if (Physics.Raycast(new Ray(new Vector3(_player.transform.position.x,0.2f, _player.transform.position.z), Vector3.left), out hit))
        {
            Debug.Log("왼쪽에 있음");
        }
        if (Physics.Raycast(new Ray(new Vector3(_player.transform.position.x, 0.2f, _player.transform.position.z), Vector3.right), out hit))
        {
            Debug.Log("오른쪽에 있음");
        }
        if (Physics.Raycast(new Ray(new Vector3(_player.transform.position.x, 0.2f, _player.transform.position.z), Vector3.back), out hit))
        {
            Debug.Log("뒤쪽에 있음");
        }
