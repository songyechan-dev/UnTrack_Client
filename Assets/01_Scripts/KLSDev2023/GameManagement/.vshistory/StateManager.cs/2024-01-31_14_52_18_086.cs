...
namespace LeeYuJoung
{
    public class StateManager : MonoBehaviourPun
    {
        ...
        public void Fire()
        {
            ...
            if (!sceneFactorys[_idx].GetComponent<FactoryManager>().isHeating && PhotonNetwork.IsMasterClient)
            {
                sceneFactorys[_idx].GetComponent<FactoryManager>().EngineOverheating();
                //히팅처리필요(others)
                object[] data = new object[] { false, sceneFactorys[_idx].GetComponent<PhotonView>().ViewID };
