        ...
        if (Physics.Raycast(transform.position, transform.forward, out hit, castRange, layerMask))
        {
            ...
            switch (hit.transform.tag)
            {
                ...
                case "DroppedTrack":
                    break;
                case "DroppedSlot":
                    if (playerController.currentTime >= playerController.spaceTime)
                    {
                        // 아이템 더미 위에 더 올리기 
                        Debug.Log("::: 더 올려 놓기 :::");
                        Debug.Log(hit.transform.tag);
                        InventoryManager _droppedSlot = hit.transform.GetComponent<InventoryManager>();
                        int num = pickSlot.transform.childCount;

                        for (int i = 0; i < num; i++)
                        {
                            _droppedSlot.DroppedSlotIn(pickSlot.transform.GetChild(0).gameObject);
