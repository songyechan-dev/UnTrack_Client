...
public class PlayerController : MonoBehaviourPunCallbacks
{
    ...
    void FixedUpdate()
    {
        ...
        if (pv == null || (pv != null && pv.IsMine && !isExit))
        {
            ...
            {
                if (transform.position.x >= MapInfo.startPosition.x && transform.position.z >= MapInfo.startPosition.z && transform.position.x <= MapInfo.endPositionX && transform.position.z <= MapInfo.endPositionZ && GameManager.Instance().gameState.Equals(GameManager.GameState.GameStart))
                {
                    PlayerMove();
                }
