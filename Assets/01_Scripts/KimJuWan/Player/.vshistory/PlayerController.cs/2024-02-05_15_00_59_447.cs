...
public class PlayerController : MonoBehaviourPunCallbacks, IPunObservable
{
    ...
    private void ChangePlayerState(PLAYERSTATE newState)
    {
        if (playerState != newState)
        {
            ...
            photonView.RPC("RPC_ChangePlayerState", RpcTarget.AllBuffered, (int)newState);
        }
    }

    [PunRPC]
    private void RPC_ChangePlayerState(int newState)
    {
        playerState = (PLAYERSTATE)newState;
        Debug.Log("Player State Changed: " + playerState);
        // 여기에서 애니메이션 상태 변경 또는 필요한 작업 수행
    }
