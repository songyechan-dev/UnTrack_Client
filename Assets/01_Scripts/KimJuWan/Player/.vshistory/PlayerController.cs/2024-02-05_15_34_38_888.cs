...
public class PlayerController : MonoBehaviourPunCallbacks
{
    ...
    {
        PhotonView.Find(_viewID).GetComponent<PlayerController>().playerState = (PLAYERSTATE)newState;

        switch (PhotonView.Find(_viewID).GetComponent<PlayerController>().playerState)
            {
                case PLAYERSTATE.IDLE:
                    playerAnim.SetInteger("PLAYERSTATE", 0);
                    break;
