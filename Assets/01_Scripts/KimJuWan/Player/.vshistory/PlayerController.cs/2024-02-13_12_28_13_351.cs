...
public class PlayerController : MonoBehaviourPunCallbacks
{
    ...
    void ViewObject()
    {
        ...
        if (Physics.Raycast(ray, out hit, 3f, layerMask))
        {
            ...
            {
                // 이전 메테리얼 복원
                foreach (var renderer in tempObject.GetComponentsInChildren<MeshRenderer>())
                {
                    Material[] materials = renderer.materials;
                    for (int i = 0; i < Mathf.Min(materials.Length, prevMaterials.Count); i++)
                    {
            ...
            else if (hit.transform.CompareTag("Item") || hit.transform.CompareTag("Plane") || hit.transform.CompareTag("DroppedSlot") || hit.transform.CompareTag("Obstacle") || hit.transform.CompareTag("DroppedTrack"))
            {
