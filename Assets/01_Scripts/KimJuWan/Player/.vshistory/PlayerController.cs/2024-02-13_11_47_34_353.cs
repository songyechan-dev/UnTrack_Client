...
public class PlayerController : MonoBehaviourPunCallbacks
{
    ...
    void CheckObject()
    {
        ...
        if (Physics.Raycast(ray, out hit, 3f, layerMask))
        {
            ...
            MeshRenderer[] meshRenderers = hit.transform.GetComponentsInChildren<MeshRenderer>();

            // 충돌된 객체가 MeshRenderer를 가지고 있다면
            if (meshRenderers.Length > 0)
            {
                // 첫 번째 메테리얼을 사용자가 지정한 매테리얼로 교체
                Material newMaterial = Instantiate(userSpecifiedMaterial);
                meshRenderers[0].materials[0] = newMaterial;

                // 나머지 메테리얼의 인덱스를 한 단계씩 늘려준다
                for (int i = 1; i < meshRenderers[0].materials.Length; i++)
                {
                    meshRenderers[0].materials[i] = meshRenderers[0].materials[i - 1];
                }
            }
        }
    }
