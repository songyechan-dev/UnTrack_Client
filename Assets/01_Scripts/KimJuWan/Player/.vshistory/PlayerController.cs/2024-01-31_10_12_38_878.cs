...
public class PlayerController : MonoBehaviourPunCallbacks
{
    ...
    void FixedUpdate()
    {
        ...
        if (pv == null || (pv != null && pv.IsMine && !isExit))
        {
            ...
            {
                if (Mathf.Ceil(transform.position.x) >= MapInfo.startPosition.x && Mathf.Ceil(transform.position.z) >= MapInfo.startPosition.z
                && Mathf.Floor(transform.position.x) <= MapInfo.endPositionX && Mathf.Floor(transform.position.z) <= MapInfo.endPositionZ
                && GameManager.Instance().gameMode.Equals(GameManager.GameMode.Play)
                && transform.position.x >= MapInfo.startPosition.x && transform.position.z >= MapInfo.startPosition.z)
                {
                    PlayerMove();
                }
