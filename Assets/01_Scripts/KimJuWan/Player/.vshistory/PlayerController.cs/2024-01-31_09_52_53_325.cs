...
public class PlayerController : MonoBehaviourPunCallbacks
{
    ...
    void FixedUpdate()
    {
        ...
        if (pv == null || (pv != null && pv.IsMine && !isExit))
        {
            ...
            {
                if ((int)transform.position.x >= MapInfo.startPosition.x && (int)transform.position.z >= MapInfo.startPosition.z 
                    && (int)transform.position.x <= MapInfo.endPositionX && (int)transform.position.z <= MapInfo.endPositionZ 
                    && GameManager.Instance().gameState.Equals(GameManager.GameState.GameStart))
                {
                    PlayerMove();
                }
                else if (!GameManager.Instance().gameState.Equals(GameManager.GameState.GameStart))
                {
                    PlayerMove();
                }
            }
