        ...
        if (pv == null || (pv != null && pv.IsMine && !isExit))
        {
            ...
            else if (!isWorking && pv != null && pv.IsMine)
            {
                if (GameManager.Instance().gameMode.Equals(GameManager.GameMode.Play))
                {
                    ...
                    if (Mathf.Ceil(newX) >= MapInfo.startPosition.x && Mathf.Ceil(newZ) >= MapInfo.startPosition.z
                        && Mathf.Floor(newX) <= MapInfo.endPositionX && Mathf.Floor(newZ) <= MapInfo.endPositionZ)
                    {
