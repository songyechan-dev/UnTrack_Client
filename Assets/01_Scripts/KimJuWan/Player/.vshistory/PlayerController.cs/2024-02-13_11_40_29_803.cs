...
public class PlayerController : MonoBehaviourPunCallbacks
{
    ...
    void CheckObject()
    {
        ...
        if (Physics.Raycast(ray, out hit, 3f, layerMask))
        {
            ...
            // 충돌된 객체의 머티리얼을 반투명하게 변경
            MeshRenderer meshRenderer = hit.transform.GetComponentInChildren<MeshRenderer>();
            if (meshRenderer != null)
            {
                // MeshRenderer가 있으면 해당 객체의 모든 머티리얼을 반투명하게 변경
                Material[] materials = meshRenderer.materials;
                foreach (Material mat in materials)
                {
                    // 머티리얼의 색상을 반투명하게 변경
                    Color color = mat.color;
                    color.a = 0.5f; // 반투명도를 조절하려면 이 값을 수정하세요
                    mat.color = color;
                }
            }
        }
