...
public class PlayerController : MonoBehaviourPunCallbacks
{
    ...
    {
        PlayerController _player = PhotonView.Find(_viewID).GetComponent<PlayerController>();
        _player.playerState = (PLAYERSTATE)newState;

        switch ((PLAYERSTATE)newState)
        {
            case PLAYERSTATE.IDLE:
                _player.playerAnim.SetInteger("PLAYERSTATE", 0);
                if (isWorking)
                {
                    _player.playerState = PLAYERSTATE.EQUIPMENTACTION;
                }
                if (isPick)
                {
                    _player.playerState = PLAYERSTATE.PICK;
                }
                if (Input.GetKeyDown(KeyCodeInfo.myActionKeyCode))
                {
                    if (!isPick)
                        _player.playerState = PLAYERSTATE.PICKUP;
                    else
                        _player.playerState = PLAYERSTATE.DROP;
                }

                break;
            case PLAYERSTATE.WALK:
                _player.playerAnim.SetInteger("PLAYERSTATE", 1);
                if (isWorking)
                {
                    _player.playerState = PLAYERSTATE.EQUIPMENTACTION;
                }
                break;
            case PLAYERSTATE.PICKUP:
                _player.playerAnim.SetInteger("PLAYERSTATE", 2);

                break;
            case PLAYERSTATE.DROP:
                _player.playerAnim.SetInteger("PLAYERSTATE", 3);
                _player.dropCurTime += Time.deltaTime;
                AnimatorClipInfo[] curClipInfo;
                curClipInfo = playerAnim.GetCurrentAnimatorClipInfo(0);
                if (dropCurTime > curClipInfo[0].clip.length)
                {
                    _player.dropCurTime = 0;
                    _player.playerState = PLAYERSTATE.IDLE;
                }
                break;
            case PLAYERSTATE.EQUIPMENTACTION:
                _player.playerAnim.SetInteger("PLAYERSTATE", 4);
                if (!_player.isWorking)
                {
                    _player.playerState = PLAYERSTATE.IDLE;
                }
                break;
            ...
            case PLAYERSTATE.PICK:
                _player.playerAnim.SetInteger("PLAYERSTATE", 5);
                if (!_player.isPick)
                {
                    _player.playerState = PLAYERSTATE.DROP;
                }
