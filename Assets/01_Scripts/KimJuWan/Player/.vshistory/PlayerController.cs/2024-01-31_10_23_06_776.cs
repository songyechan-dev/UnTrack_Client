    ...
    void FixedUpdate()
    {
        ...
        if (pv == null || (pv != null && pv.IsMine && !isExit))
        {
            ...
            else if (!isWorking && pv != null && pv.IsMine)
            {
                ...
                {
                    float newX = Mathf.Clamp(transform.position.x, MapInfo.startPosition.x, MapInfo.endPositionX);
                    float newZ = Mathf.Clamp(transform.position.z, MapInfo.startPosition.z, MapInfo.endPositionZ);

                    if (newX == Mathf.Clamp(newX, MapInfo.startPosition.x, MapInfo.endPositionX) &&
                        newZ == Mathf.Clamp(newZ, MapInfo.startPosition.z, MapInfo.endPositionZ))
                    {
                        transform.position = new Vector3(newX, transform.position.y, newZ);
                        PlayerMove();
                        Debug.Log(MapInfo.startPosition);
                        Debug.Log(MapInfo.endPosition.x);
                        Debug.Log(MapInfo.endPosition.y);
                        Debug.Log(MapInfo.endPosition.z);
