            ...
            switch (hit.transform.tag)
            {
                ...
                case "Item":
                    ...
                    if (!playerController.isPick)
                    {
                        ...
                        photonView.RPC("PickedItemSetPosition_Sync",RpcTarget.All,pickSlot.GetChild(0).transform.position, pickSlot.position);
                    }
                    else
                    {
                            ...
                            inventoryManager.itemType.Equals(ItemManager.ITEMTYPE.DROPPEDTRACK))
                        {
                            ...
                            {
                                hit.transform.SetParent(pickSlot.transform);
                                photonView.RPC("PickedItemSetPosition_Sync", RpcTarget.All, pickSlot.GetChild(inventoryManager.itemNum - 1).transform.position, pickSlot.position + (Vector3.up * (inventoryManager.itemNum - 1)));
                            }
                        }                                     
                    }
