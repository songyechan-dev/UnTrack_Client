...
public class PlayerManager : MonoBehaviourPunCallbacks
{
    ...
    public void Test(Transform _tr, Vector3 _targetPos)
    {
        _tr.position = _targetPos;
        photonView.RPC("Test_Sync", RpcTarget.Others, _tr, _targetPos);
    }
    [PunRPC]
    public void Test_Sync(Transform _tr, Vector3 _targetPos)
    {
        _tr.position = _targetPos;
    }
            ...
            switch (hit.transform.tag)
            {
                ...
                case "Item":
                    ...
                    if (!playerController.isPick)
                    {
                        ...
                        Test(pickSlot.GetChild(0).transform, pickSlot.position);
                    }
                    else
                    {
