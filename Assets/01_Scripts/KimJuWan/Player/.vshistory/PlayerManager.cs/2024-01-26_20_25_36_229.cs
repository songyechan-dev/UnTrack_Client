...
public class PlayerManager : MonoBehaviourPunCallbacks
{
    ...
    void SetParentOnNetwork(int viewId, int parentViewId)
    {
        ...
        Debug.Log("second :: " + newParentView);

        if (viewToChange != null && newParentView != null)
        {
            Transform pickSlotTransform = viewToChange.gameObject.transform.Find("PickSlot");

            // PickSlot을 찾았고, 자식이 있는 경우에만 SetParent를 시도
            if (pickSlotTransform != null && pickSlotTransform.childCount > 0)
            {
                // 변경된 정보를 적용
                pickSlotTransform.GetChild(0).SetParent(newParentView.transform);
            }
            else
            {
                Debug.LogWarning("PickSlot or its child not found.");
            }
        }
        else
        {
            Debug.LogWarning("PhotonViews not found.");
        }
    }
