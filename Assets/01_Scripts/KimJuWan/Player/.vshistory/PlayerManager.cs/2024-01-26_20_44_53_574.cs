                    ...
                    if (playerController.isPick)
                    {
                        if (inventoryManager.itemType.Equals(ItemManager.ITEMTYPE.WOOD) || inventoryManager.itemType.Equals(ItemManager.ITEMTYPE.STEEL))
                        {
                            ...
                            for (int i = 0; i < num; i++)
                            {
                                ...
                                PhotonNetwork.Instantiate(inventoryManager.itemType.ToString(), picChild.transform.position, picChild.transform.rotation).transform.SetParent(_parentTransform);
