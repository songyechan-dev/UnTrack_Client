...
public class PlayerManager : MonoBehaviourPunCallbacks
{
            ...
            switch (hit.transform.tag)
            {
                ...
                case "Item":
                    ...
                    if (!playerController.isPick)
                    {
                        ...
                        if (!inventoryManager.itemType.Equals(ItemManager.ITEMTYPE.AX) && !inventoryManager.itemType.Equals(ItemManager.ITEMTYPE.PICK))
                        {
    ...
    void SetItemPos(ItemManager.ITEMTYPE _itemType,bool _isPickUP,int _viewID,int hitViewID = -1)
    {
        ...
        GameObject _hit = PhotonView.Find(hitViewID).gameObject;
        ...
        else
        {
            if (_itemType.Equals(ItemManager.ITEMTYPE.AX))
            {
                ...
                go.transform.GetComponent<PlayerManager>(). pickSlot.transform.GetChild(0).SetParent(null);
            }
            else
            {
                go.transform.Find("PickSlot").transform.GetChild(0).GetChild(0).GetComponentInChildren<MeshRenderer>().enabled = true;
