...
public class PlayerManager : MonoBehaviourPunCallbacks
{
    ...
    public void Test(Transform _tr)
    {
        if (photonView.IsMine)
        {
            _tr.transform.SetParent(pickSlot.transform);
            pickSlot.GetChild(0).transform.position = pickSlot.position;
            photonView.RPC("Test_Sync", RpcTarget.Others, _tr);
        }
    }

    [PunRPC]
    public void Test_Sync(Transform _tr)
    {
        _tr.SetParent(pickSlot.transform);
        pickSlot.GetChild(0).transform.position = pickSlot.position;
    }
