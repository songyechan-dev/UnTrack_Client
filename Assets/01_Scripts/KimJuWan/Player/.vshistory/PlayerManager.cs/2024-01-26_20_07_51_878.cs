...
public class PlayerManager : MonoBehaviourPunCallbacks
{
        ...
        if (Physics.Raycast(ray, out hit, castRange, layerMask))
        {
            ...
            switch (hit.transform.tag)
            {
                ...
                case "DroppedSlot":
                    ...
                    //                {
                    //                    hit.transform.GetChild(i).gameObject.GetComponent<PhotonView>().TransferOwnership(photonView.OwnerActorNr);
                    //                }
                    //            }
                    ...
                    else
                    {
                        ...
                        else
                        {
                            ...
                            inventoryManager.SaveInventory(hit.transform.GetChild(droppedSlot.itemNum - 1).gameObject);
                            //hit.transform.GetChild(droppedSlot.itemNum - 1).SetParent(pickSlot); // ::::::::
                            PropagateParentChange_Dropped(hit.transform.GetComponent<PhotonView>().ViewID, photonView.ViewID);
                            pickSlot.GetChild(0).transform.position = pickSlot.position;
                            droppedSlot.DroppedSlotOut();
                        }
                    ...
                    if (playerController.isPick)
                    {
                        if (inventoryManager.itemType.Equals(ItemManager.ITEMTYPE.WOOD) || inventoryManager.itemType.Equals(ItemManager.ITEMTYPE.STEEL))
                        {
                            ...
                            for (int i = 0; i < num; i++)
                            {
                                ...
                                PropagateParentChange(photonView.ViewID, _droppedSlot.GetComponent<PhotonView>().ViewID);
    ...
    [PunRPC]
    void SetParentOnNetwork(int viewId, int parentViewId)
    {
        // 이 코드는 모든 클라이언트에서 실행
        // viewId: 변경할 객체의 PhotonView ID
        // parentViewId: 새로운 부모 객체의 PhotonView ID

        PhotonView viewToChange = PhotonView.Find(viewId);
        PhotonView newParentView = PhotonView.Find(parentViewId);

        Debug.Log("first::::"+viewToChange);
        Debug.Log("secound :: "+newParentView);
        if (viewToChange != null && newParentView != null)
        {
            // 변경된 정보를 적용
            viewToChange.gameObject.transform.Find("PickSlot").GetChild(0).SetParent(newParentView.transform);
        }
    }

    void PropagateParentChange(int viewId, int parentViewId)
    {
        // RPC를 사용하여 변경된 정보를 다른 플레이어에게 전송
        photonView.RPC("SetParentOnNetwork", RpcTarget.All, viewId, parentViewId);
    }

    [PunRPC]
    void SetParentOnNetwork_Dropped(int viewId, int parentViewId)
    {
        // 이 코드는 모든 클라이언트에서 실행
        // viewId: 변경할 객체의 PhotonView ID
        // parentViewId: 새로운 부모 객체의 PhotonView ID
        ...
        {
            // 변경된 정보를 적용
            viewToChange.gameObject.transform.GetChild(viewToChange.gameObject.GetComponent<InventoryManager>().itemNum - 1).SetParent(newParentView.transform);
        }
    }

    void PropagateParentChange_Dropped(int viewId, int parentViewId)
    {
        // RPC를 사용하여 변경된 정보를 다른 플레이어에게 전송
        photonView.RPC("SetParentOnNetwork_Dropped", RpcTarget.All, viewId, parentViewId);
    }
