...
public class PlayerManager : MonoBehaviourPun
{
    ...
    public void PickedItemSetPosition(Vector3 _originPos, Vector3 _targetPos)
    {
        _originPos = _targetPos;
        photonView.RPC("PickedItemSetPosition_Sync", RpcTarget.Others, _originPos, _targetPos);
    }
    ...
    public void CollectIngredient()
    {
        ...
        if (Physics.Raycast(ray, out hit, castRange, layerMask))
        {
            ...
            switch (hit.transform.tag)
            {
                ...
                case "Item":
                    Debug.Log("호출됨");
                    if (!playerController.isPick && photonView.IsMine)
                    {
