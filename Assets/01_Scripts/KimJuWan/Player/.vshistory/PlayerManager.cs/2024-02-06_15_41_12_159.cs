                ...
                case "Plane":
                    ...
                    if (playerController.isPick)
                    {
                        ...
                        if (inventoryManager.itemType.Equals(ItemManager.ITEMTYPE.WOOD) || inventoryManager.itemType.Equals(ItemManager.ITEMTYPE.STEEL))
                        {
                            playerController.isPick = false;
                            castRange = 1.0f;
                            GameObject _droppedSlot = PhotonNetwork.Instantiate("DroppedSlot", new Vector3(pickSlot.transform.position.x, pickSlot.transform.position.y +1f, pickSlot.transform.position.z), Quaternion.identity);
