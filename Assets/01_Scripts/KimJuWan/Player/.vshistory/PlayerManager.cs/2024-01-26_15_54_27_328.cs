        ...
        if (Physics.Raycast(ray, out hit, castRange, layerMask))
        {
            ...
            switch (hit.transform.tag)
            {
                ...
                case "DroppedTrack":
                    ...
                    break;
                case "DroppedSlot":
                    if (hit.transform.GetComponent<PhotonView>().transform.childCount <= 0)
                    {
                        hit.transform.GetComponent<PhotonView>().TransferOwnership(photonView.OwnerActorNr); // 소유자 변경
                    }
                    else
                    {
                        ...
                        {
                            hit.transform.GetComponentsInChildren<PhotonView>()[i].TransferOwnership(photonView.OwnerActorNr); // 소유자 변경
                        }
                    }
