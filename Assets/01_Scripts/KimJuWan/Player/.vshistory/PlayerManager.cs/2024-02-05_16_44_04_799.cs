...
public class PlayerManager : MonoBehaviourPunCallbacks
{
            ...
            switch (hit.transform.tag)
            {
                ...
                case "Item":
                    ...
                    if (!playerController.isPick)
                    {
                        ...
                        {
                            photonView.RPC("SetItemPos", RpcTarget.All, ItemManager.ITEMTYPE.AX,true,photonView.ViewID);
                        ...
                        {
                            photonView.RPC("SetItemPos", RpcTarget.All, ItemManager.ITEMTYPE.PICK,true,photonView.ViewID);
                            //playerController.playerState = PlayerController.PLAYERSTATE.IDLE;
                            //pickSlot.GetChild(0).GetChild(0).GetComponentInChildren<MeshRenderer>().enabled = false;
                ...
                case "Plane":
                    ...
                    if (playerController.isPick)
                    {
                        ...
                        else if (inventoryManager.itemType.Equals(ItemManager.ITEMTYPE.AX))
                        {
                            ...
                            photonView.RPC("SetItemPos", RpcTarget.All, ItemManager.ITEMTYPE.AX, false, photonView.ViewID);
                        ...
                        else if (inventoryManager.itemType.Equals(ItemManager.ITEMTYPE.PICK))
                        {
                            playerController.isPick = false;
                            castRange = 1.0f;
                            photonView.RPC("SetItemPos", RpcTarget.All, ItemManager.ITEMTYPE.PICK, false, photonView.ViewID);
                            //pickSlot.transform.GetChild(0).GetChild(0).GetComponentInChildren<MeshRenderer>().enabled = true;
    ...
    [PunRPC]
    void SetItemPos(ItemManager.ITEMTYPE _itemType,bool _isPickUP,int _viewID)
    {
        ...
        if (_isPickUP)
        {
            ...
            {
                playerController.playerState = PlayerController.PLAYERSTATE.IDLE;
                go.transform.GetComponent<PlayerManager>().pickSlot.GetChild(0).GetChild(0).GetComponentInChildren<MeshRenderer>().enabled = false;
