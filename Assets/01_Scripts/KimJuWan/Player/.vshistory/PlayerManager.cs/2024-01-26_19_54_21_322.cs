...
public class PlayerManager : MonoBehaviourPunCallbacks
{
                    ...
                    if (playerController.currentTime >= playerController.spaceTime && playerController.isPick)
                    {
                        ...
                        if (inventoryManager.itemType.Equals(droppedSlot.itemType))
                        {
                            ...
                            for (int i = 0; i < n; i++)
                            {
                                ...
                                PropagateParentChange(photonView.ViewID, droppedSlot.GetComponent<PhotonView>().ViewID);
    ...
    void SetParentOnNetwork(int viewId, int parentViewId)
    {
        ...
        {
            // 변경된 정보를 적용
            viewToChange.gameObject.transform.Find("PickSlot").SetParent(newParentView.transform);
        }
    }
