        ...
        if (Physics.Raycast(ray, out hit, castRange, layerMask))
        {
            ...
            switch (hit.transform.tag)
            {
                ...
                case "DroppedSlot":
                    ...
                    //for (int i = 0; i < childCount; i++)
                    //{
                    //    if (hit.transform.GetChild(i).transform.GetComponent<PhotonView>() != null && hit.transform.GetChild(i).transform.GetComponent<PhotonView>().OwnerActorNr != photonView.OwnerActorNr)
                    //    {
                    //        hit.transform.GetChild(i).transform.GetComponent<PhotonView>().TransferOwnership(photonView.OwnerActorNr); // 소유자 변경
                    //    }
                    //}
