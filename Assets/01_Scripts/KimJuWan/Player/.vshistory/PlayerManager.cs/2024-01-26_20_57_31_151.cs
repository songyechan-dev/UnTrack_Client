                    ...
                    if (playerController.isPick)
                    {
                        if (inventoryManager.itemType.Equals(ItemManager.ITEMTYPE.WOOD) || inventoryManager.itemType.Equals(ItemManager.ITEMTYPE.STEEL))
                        {
                            ...
                            for (int i = 0; i < num; i++)
                            {
                                ...
                                Transform picChild = pickSlot.transform.GetChild(0)?.transform;

                                if (picChild != null && _droppedSlot != null)
                                {
                                    GameObject instantiatedObject = PhotonNetwork.Instantiate(inventoryManager.itemType.ToString(), picChild.position, picChild.rotation);

                                    // 새로 생성된 오브젝트의 부모를 _droppedSlot으로 설정
                                    instantiatedObject.transform.SetParent(_droppedSlot.transform);

                                    // picChild 게임 오브젝트 파괴
                                    PhotonNetwork.Destroy(picChild.gameObject);
                                }
                                else
                                {
                                    Debug.LogWarning("picChild or _droppedSlot is null.");
                                }
